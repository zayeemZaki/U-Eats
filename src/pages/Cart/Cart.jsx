...
import { Footer } from '../../container';
import { Navbar } from '../../components';
import { API } from 'aws-amplify';  // Import the API module from AWS Amplify
import './Cart.css';
...
const Cart = ({ cart, setCartData }) => {
  ...
  const handleCheckOut = async () => {
    try {
      // Use the Amplify API.post method to make the API call
      const response = await API.post('api391498c0', '/create-checkout-session', {
        body: { cart },
      });
  ...
  return (
    <div>
      ...
      <div className="app__cart">
          ...
          {cart.map((item, index) => (
            <div key={index}>
              ...
              <p>
                ...
                <button type="button" className="custom__button-Minus" onClick={() => handleMinusQuantity(index)}>
                  {' '}
                  -{' '}
                </button>
                <button type="button" className="custom__button-Add" onClick={() => handleAddQuantity(index)}>
                  {' '}
                  +{' '}
                </button>
              </p>
        ...
        <p className="app__cart-total flex__right section__padding">Total Price: {formattedTotalPrice}</p>
